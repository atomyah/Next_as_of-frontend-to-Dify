...
// バリデーションエラー処理
function handleValidationError(error: ZodError): ActionState {  // Promise<ActionState>で必ずActionStateを返す契約のため戻り値の型は必ずActionState
  ...
  // カスタムエラー処理
  function handleError(customErrors: Record<string, string[]>): ActionState { // Promise<ActionState>で必ずActionStateを返す契約のため戻り値の型は必ずActionState
...
export async function createUser(
    ...
    ): Promise<ActionState> // 戻り値: 非同期でActionStateを返し、この戻り値が次回のprevState になる
                            // Promise:非同期処理の結果を表す型. async function は必ず Promise を返す.
                            // 「この関数は非同期で実行され、完了時にActionState形式のデータを返しますよ」という契約
{
    ...
    // バリデーション
    const validationResult = registerSchema.safeParse(rawFormData) //rawFormData:フォームから渡ってきた情報。safeParse: src/validation/user.tsに定義したZodバリデーションを実行
